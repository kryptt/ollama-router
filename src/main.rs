...
async fn model_route(
    ...
    let parsed = match proxy::parse_request(&body) {
        Some(p) => p,
        None => return proxy::bad_request("request body must contain a non-empty 'model' field"),
    };

    let reg = state.registry.read().await;
    let backend_id = match reg.lookup(&parsed.model) {
        Some(id) => id,
        None => {
            state.metrics.unknown_model_requests.inc();
            let available = reg.available_model_names();
            return proxy::model_not_found(&parsed.model, &available);
        }
    };
